"""Leveranciers: van bron-JSON naar rijen voor de tabel ``supplier``.
...
def parse_supplier_list(
    json: Union[Mapping[str, Any], Sequence[Mapping[str, Any]]]
) -> List[Dict[str, Any]]:
    """Accepteert een volledige Laravel-paginapayload of een kale itemlijst."""
    if isinstance(json, Mapping):
        items = json.get("data") or []
    else:
        items = json
    return [
        {column: item.get(column) for column in SUPPLIER_COLUMNS}
        for item in items
    ]
